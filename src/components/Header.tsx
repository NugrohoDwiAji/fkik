  ...
  return (
    <nav
      className={`h-16 md:h-20 flex items-center justify-between fixed  left-0 w-full text-shadow-lg  z-50 ${
        isScroll ? "shadow-lg bg-white text-green-800 top-0 " : "top-7 lg:top-16 text-white "
    ...
    >
      <div className="flex items-center gap-3 ">
        ...
        <div className={`  font-bold text-xs md:text-sm lg:text-base`}>
          ...
          <hr className={isScroll ?"text-green-800":"text-white"} />
          <h1>Universitas Bumigora</h1>
        </div>
      </div>
      <AlignRight
        size={30}
        className={`text-green-800 hover:text-white ease-in-out duration-300 transition-all md:hidden `}
          ...
          <div key={item.id}>
            <Link
              ...
              className={classnames({
                "underline font-bold text-green-900":
                  currentPath === item.url ||
                  item.subMenu?.some((path) => path.url === currentPath),
                "border-none text-green-800" : currentPath !== item.url && isScroll === true,
                " hover:text-green-900 t hover:underline hover:text-shadow-lg/20 hover:font-bold hover:shadow-lg ease-in-out duration-300 transition-all ":
                  true,
              })}
              onClick={item.action}
            >
            ...
            {subActive === item.name && (
              <div
                className={`flex flex-col gap-3 absolute mt-2 bg-gradient-to-r from-green-600 to-green-800  rounded-lg ${
      ...
      <div
        className={` ${
          isActive ? "flex absolute z-50" : "hidden"
        } top-0 text-white right-0 flex-col bg-gradient-to-r from-yellow-600 to-green-800 gap-3 px-5 py-4 rounded-bl-xl w-60`}
      >
